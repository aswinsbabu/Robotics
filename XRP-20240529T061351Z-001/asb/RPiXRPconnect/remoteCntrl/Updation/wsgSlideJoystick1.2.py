...
def home():
    ...
    <body>
        ...
        <script>
            let lastCommandTime = 0;  // Store the time of the last command
            ...
            function sendCommand(command) {
                ...
                // Only send the command if 1 second has passed since the last command
...
'''   Operation
Only send one command in one sec
Neglect multiple swipe inputs

Explanation of Changes:
Throttle command sending with a timer: We introduced a JavaScript lastCommandTime variable and a COMMAND_DELAY of 1000ms (1 second). This ensures the joystick only sends a command if at least one second has passed since the last command.
Joystick movement: When the joystick is moved, the system checks the time since the last command was sent and only sends a new command if the time has elapsed.
This should prevent multiple commands from being sent in quick succession and only allow one command per second based on joystick movement.
'''
