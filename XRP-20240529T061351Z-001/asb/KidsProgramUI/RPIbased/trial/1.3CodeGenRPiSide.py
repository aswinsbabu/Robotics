...
<body>
    ...
    <script>
        let commandList = [];
        const codeSnippets = {
            forward: `drivetrain.set_effort(0.8, 0.8)
time.sleep(drive_time)
drivetrain.stop()`,
            backward: `drivetrain.set_effort(-0.8, -0.8)
time.sleep(drive_time)
drivetrain.stop()`,
            left: `drivetrain.set_effort(-0.8, 0.8)
time.sleep(drive_time)
drivetrain.stop()`,
            right: `drivetrain.set_effort(0.8, -0.8)
time.sleep(drive_time)
drivetrain.stop()`,
...
drivetrain.stop()`
        ...
        function updateCodeViewer() {
            let code = commandList.map(cmd => codeSnippets[cmd] || '').join('\\n\\n');
